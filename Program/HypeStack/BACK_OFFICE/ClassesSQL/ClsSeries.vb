...
Public Class ClsSeries
    ...
    Friend Shared Function Cria_Serie(pAno_Trabalho As String) As Boolean
        ...
        mLista_TD = ClsTipos_Doc.GetTipos_DocByES("X")
        ...
        For Each pTD As ClsTipos_Doc In mLista_TD
            ...
            '-------------------------------------
            'Gravar a serie na tablea de Tipos_Doc
            '-------------------------------------
    ...
    Public Shared Function FiltrarSeries(ByVal pFiltro As ClsSeries) As List(Of ClsSeries)
        ...
        mLista = mListaSeries.FindAll(Function(x) _
        x.Cod_Serie.Contains(pFiltro.Cod_Serie)
        )
        '-----------------------------------------------------
    ...
    Public Shared Function GetSeriesByAno(ByVal pAno As String) As List(Of ClsSeries)
        Dim mListaSeries As New List(Of ClsSeries)
        ...
        mDB.ConnectionString = My.Settings.Connection
        mCMD.Parameters.Clear()
        mCMD.Parameters.AddWithValue("@pAno", pAno)


        mStrSQL = "SELECT "
        mStrSQL += "Series.Cod_Serie,"
        mStrSQL += "Series.FLG_Tipo_Doc,"
        mStrSQL += "Tipos_Doc.Designacao, "
        mStrSQL += "Series.Ano_Trabalho,"
        mStrSQL += "Series.Contador,"
        mStrSQL += "Series.FLG_Ativo "
        mStrSQL += "FROM Series "
        mStrSQL += "INNER JOIN Tipos_Doc "
        mStrSQL += "ON Series.FLG_Tipo_Doc = Tipos_Doc.FLG_Tipo_Doc "
        mStrSQL += "WHERE Series.Ano_Trabalho=@pAno"
...
Public Class Series2
    ...
    Public Property Contador As Integer                                     ' SQL_Type: smallint
    Public Property FLG_Ativo As Boolean                                    ' SQL_Type: bit
    Public Property Designacao As String                                    ' SQL_Type: String


    Public Shared Function GetSeriesByAno(ByVal pAno As String) As List(Of Series2)
        Dim mListaSeries As New List(Of Series2)
